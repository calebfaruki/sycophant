...
**The pods that execute LLM-driven code (harness, llm-job, airlock-job) are adversarial, and the rest of the cluster stays safe regardless of what they do.**
...
1. **Exfiltrate**: reach network destinations its chamber didn't declare. This is enforced by a Cilium L7 DNS FQDN allowlist, egress default-deny on the harness, and per-component CiliumNetworkPolicies on airlock-job and llm-job.
