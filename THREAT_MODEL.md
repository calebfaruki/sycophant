...
3. **Impersonate** — present as a different workspace, tenant, or trusted in-cluster service. Audience-bound SA tokens (KEP-1205) with one audience per pair (transponder→tightbeam, transponder→airlock, llm-job→tightbeam), server-minted `channel_id`, P-256 `ClientSignatureVerifier` on the external surface.
