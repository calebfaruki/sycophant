...
use model_provider::{
    collect_text, collect_thinking, collect_tool_calls, stream_event_to_chunk, LlmProvider,
...
use toolset_proto::{AwaitTurnCancelRequest, GetTurnRequest};
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let controller_addr =
        std::env::var("TOOLSET_CONTROLLER_ADDR").unwrap_or_else(|_| "http://127.0.0.1:9090".into());

    let model_name = std::env::var("TOOLSET_MODEL_NAME").map_err(|_| {
        "TOOLSET_MODEL_NAME env var is required (set by the controller when spawning the prompt Job)"
    })?;
    ...
    let mut client =
        ToolsetControllerClient::with_interceptor(channel, SaTokenInterceptor::default_path());
...
async fn send_error_chunk(
    tx: &mut futures::channel::mpsc::Sender<toolset_proto::TurnResultChunk>,
    scrub_set: &ScrubSet,
    message: String,
) {
    let mut chunk = toolset_proto::TurnResultChunk {
        chunk: Some(toolset_proto::turn_result_chunk::Chunk::Error(
            toolset_proto::TurnError { code: -1, message },
        )),
    };
...
async fn open_or_timeout(
    ...
    scrub_set: &ScrubSet,
    tx: &mut futures::channel::mpsc::Sender<toolset_proto::TurnResultChunk>,
) -> Option<std::pin::Pin<Box<dyn futures::Stream<Item = Result<StreamEvent, String>> + Send>>> {
    match tokio::time::timeout(
...
async fn drain_stream(
    mut stream: std::pin::Pin<Box<dyn futures::Stream<Item = Result<StreamEvent, String>> + Send>>,
    tx: &mut futures::channel::mpsc::Sender<toolset_proto::TurnResultChunk>,
    scrub_set: &ScrubSet,
    ...
    loop {
        tokio::select! {
            ...
            maybe = stream.next() => match maybe {
                Some(Ok(event)) => {
                    ...
                    let drop_chunk = matches!(
                        chunk.chunk,
                        Some(toolset_proto::turn_result_chunk::Chunk::Complete(_)) | None
                    );
                    if !drop_chunk {
                        scrub_chunk(&mut chunk, scrub_set);
                        let _ = tx.send(chunk).await;
            ...
            _ = heartbeat.tick() => {
                let _ = tx
                    .send(toolset_proto::TurnResultChunk {
                        chunk: Some(toolset_proto::turn_result_chunk::Chunk::ContentDelta(
                            toolset_proto::ContentDelta { text: String::new() },
                        )),
                    })
                    .await;
            }
    ...
    let thinking = collect_thinking(&events);

    let mut final_content: Vec<toolset_proto::ContentBlock> = Vec::new();
    if let Some(t) = thinking {
        final_content.push(toolset_proto::ContentBlock {
            block: Some(toolset_proto::content_block::Block::Thinking(
                toolset_proto::ThinkingBlock { text: t },
            )),
        });
    }
    if let Some(t) = text {
        final_content.push(toolset_proto::ContentBlock {
            block: Some(toolset_proto::content_block::Block::Text(
                toolset_proto::TextBlock { text: t },
            )),
        });
    }
    let mut complete = toolset_proto::TurnResultChunk {
        chunk: Some(toolset_proto::turn_result_chunk::Chunk::Complete(
...
async fn process_turn(
    ...
    client: &mut ToolsetControllerClient<
    ...
    let (mut tx, rx) = futures::channel::mpsc::channel::<toolset_proto::TurnResultChunk>(64);
    ...
    let sender = async {
        ...
        if let Ok(val) = model_name.parse() {
            request.metadata_mut().insert("x-toolset-model", val);
        }
...
mod tests {
    ...
    async fn open_time_error_emits_one_turn_error_chunk() {
        ...
        let (mut tx, mut rx) = futures::channel::mpsc::channel::<toolset_proto::TurnResultChunk>(4);
        ...
        match chunk.chunk {
            Some(toolset_proto::turn_result_chunk::Chunk::Error(e)) => {
                assert_eq!(e.code, -1);
