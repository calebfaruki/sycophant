//! Storage interface for Enrollment CR access. The `EnrollmentStore`
//! trait decouples the `redeem_for_enrollment` business logic from
//! `kube::Api` so unit tests can inject a stub that returns canned
//! Enrollment states — covering the four branches (invalid input,
//! not-found, already-enrolled, success) without a real cluster.
//!
//! The production impl `KubeEnrollmentStore` wraps
...
use crate::enrollment_watcher::{build_redeem_patch, FIELD_MANAGER};

#[async_trait]
pub trait EnrollmentStore: Send + Sync {
    /// Fetch an Enrollment CR by name. `Ok(None)` means 404; `Err` for
    /// other failures.
    async fn get(&self, name: &str) -> Result<Option<Enrollment>, Status>;
    /// SSA-patch the Enrollment's status subresource using `FIELD_MANAGER`.
    async fn patch_status(&self, name: &str, patch: &serde_json::Value) -> Result<(), Status>;
}

pub struct KubeEnrollmentStore {
    api: Api<Enrollment>,
}

impl KubeEnrollmentStore {
...
/// Map a kube fetch result into the `EnrollmentStore::get` contract:
...
fn map_kube_get(result: Result<Enrollment, kube::Error>) -> Result<Option<Enrollment>, Status> {
    match result {
        Ok(c) => Ok(Some(c)),
...
#[async_trait]
impl EnrollmentStore for KubeEnrollmentStore {
    async fn get(&self, name: &str) -> Result<Option<Enrollment>, Status> {
...
pub async fn redeem_for_enrollment(
    ...
    if current.status.and_then(|s| s.public_key).is_some() {
        return Err(Status::failed_precondition(
            "enrollment already enrolled; rotate via status patch first",
        ));
    }
...
mod tests {
    ...
    use crate::crd::{EnrollmentSpec, EnrollmentStatus};
    ...
    /// failure on the read.
    struct StubEnrollmentStore {
        fetched: Mutex<Option<Enrollment>>,
        get_error: Mutex<Option<Status>>,
        patches: Mutex<Vec<(String, serde_json::Value)>>,
    }

    impl StubEnrollmentStore {
        fn ok(fetched: Option<Enrollment>) -> Self {
            Self {
    ...
    #[async_trait]
    impl EnrollmentStore for StubEnrollmentStore {
        async fn get(&self, _name: &str) -> Result<Option<Enrollment>, Status> {
    ...
    fn unregistered_enrollment(name: &str) -> Enrollment {
        Enrollment {
            metadata: ObjectMeta {
                name: Some(name.into()),
                ..Default::default()
    ...
    fn already_enrolled_enrollment(name: &str) -> Enrollment {
        Enrollment {
            metadata: ObjectMeta {
                name: Some(name.into()),
                ..Default::default()
    ...
    async fn redeem_with_invalid_sec1_public_key_returns_invalid_argument() {
        ...
        let err = redeem_for_enrollment(&store, &claims("alpha"), b"not a key")
            .await
            .unwrap_err();
    ...
    async fn redeem_with_empty_public_key_returns_invalid_argument() {
        let store = StubEnrollmentStore::ok(Some(unregistered_enrollment("alpha")));
        let err = redeem_for_enrollment(&store, &claims("alpha"), &[])
            .await
            .unwrap_err();
        assert_eq!(err.code(), tonic::Code::InvalidArgument);
    ...
    #[tokio::test]
    async fn redeem_with_missing_enrollment_cr_returns_not_found() {
        let store = StubEnrollmentStore::ok(None);
        let err = redeem_for_enrollment(&store, &claims("ghost"), &fresh_sec1())
            .await
            .unwrap_err();
    ...
    #[tokio::test]
    async fn redeem_with_unregistered_enrollment_persists_public_key_and_clears_code() {
        use base64::Engine;
        let store = StubEnrollmentStore::ok(Some(unregistered_enrollment("alpha")));
        let sec1 = fresh_sec1();
    ...
    async fn redeem_propagates_store_get_error() {
        let store = StubEnrollmentStore::get_error(Status::internal("kube down"));
        let err = redeem_for_enrollment(&store, &claims("alpha"), &fresh_sec1())
            .await
            .unwrap_err();
    ...
    fn dummy_enrollment() -> Enrollment {
        Enrollment {
            metadata: ObjectMeta {
    ...
    fn map_kube_get_ok_returns_some() {
        let r = map_kube_get(Ok(dummy_enrollment())).unwrap();
        assert!(r.is_some());
    }
