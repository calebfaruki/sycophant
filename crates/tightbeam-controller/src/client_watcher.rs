//! Client CR watcher: mints one-time enrollment codes for fresh
//! Clients, installs registered public keys into the
//! `ClientSignatureVerifier` cache, and removes them on delete.
//!
//! Two responsibilities:
//!
//! 1. **Mint codes.** When a Client appears with no `status.publicKey`
//!    and no in-flight `status.enrollmentCode`, sign a one-time code
//!    with the controller's signing key and SSA-patch it onto status.
//!    Operators see it via `kubectl get tbcl -o yaml` and deliver it
//!    out-of-band to the device.
//!
//! 2. **Install public keys.** When a Client carries a registered
...
//!
//! `redeem_enrollment` (in `grpc.rs`) writes `publicKey` + `enrolledAt`
...
use crate::crd::{Client, ClientSpec, ClientStatus};

/// SSA field manager used by every status write the controller performs
/// on Client CRs. Both this watcher (minting codes) and the
...
/// Pure decision: given a Client's current status, what should the
...
pub fn decide_action(status: Option<&ClientStatus>) -> ClientAction {
    let Some(status) = status else {
        return ClientAction::MintCode;
    };
    if status.public_key.is_some() {
        return ClientAction::InstallKey;
    }
    if status.enrollment_code.is_some() {
        return ClientAction::NoOp;
    }
    ClientAction::MintCode
}
...
/// as "skip this Client" rather than crash the watcher.
...
/// the Client's status subresource. Returns a complete-enough document
/// for `Patch::Apply` against `patch_status`.
...
pub fn registration_from(spec: &ClientSpec, vk: VerifyingKey) -> ClientRegistration {
    ClientRegistration {
        verifying_key: vk,
...
/// Watch Client CRs in `namespace` and keep the verifier's cache in
/// sync, minting codes for fresh Clients along the way. `ready_tx`
/// signals when the initial sync has completed (mirrors the model /
/// provider watcher pattern so main.rs can join on all three).
pub async fn watch_clients(
    ...
    let api: Api<Client> = Api::namespaced(client, namespace);
    ...
    while let Some(event) = stream
        .try_next()
        .await
        .map_err(|e| format!("client watcher error: {e}"))?
    {
        match event {
            Event::Init => {
                tracing::info!("client watcher initialized");
            ...
            Event::Delete(cr) => {
                ...
                tracing::info!(client = %name, "client deleted");
                registrations.write().await.remove(&name);
            }
            Event::InitDone => {
                tracing::info!("client watcher initial sync complete");
                let _ = ready_tx.send(true);
            }
        }
    }

    tracing::warn!("client watcher stream ended");
    Ok(())
}
...
async fn handle_apply(
    ...
    let Some(name) = cr.metadata.name.clone() else {
        tracing::warn!("client has no name; skipping");
        return;
    };

    match decide_action(cr.status.as_ref()) {
        ClientAction::InstallKey => {
            let Some(b64) = cr.status.as_ref().and_then(|s| s.public_key.as_ref()) else {
                return;
            };
            let Some(vk) = parse_public_key_b64(b64) else {
                tracing::warn!(client = %name, "client publicKey malformed; not installing");
                return;
            };
            tracing::info!(client = %name, "installing client public key");
            registrations
                .write()
                .await
        ...
        ClientAction::MintCode => {
            ...
            match api.patch_status(&name, &pp, &Patch::Apply(&patch)).await {
                Ok(_) => tracing::info!(client = %name, "minted enrollment code"),
                Err(e) => tracing::error!(client = %name, "patch_status failed: {e}"),
            }
        }
        ClientAction::NoOp => {
            tracing::debug!(client = %name, "client awaiting redeem");
        }
    }
}
...
mod tests {
    ...
    fn fresh_status_with_public_key(b64: &str) -> ClientStatus {
        ClientStatus {
    ...
    fn decide_action_with_no_status_returns_mint_code() {
        assert_eq!(decide_action(None), ClientAction::MintCode);
    }

    #[test]
    fn decide_action_with_default_status_returns_mint_code() {
        // ClientStatus::default has no publicKey and no enrollmentCode
        // — the fresh-Client case before the watcher has touched it.
        let status = ClientStatus::default();
        assert_eq!(decide_action(Some(&status)), ClientAction::MintCode);
    }

    #[test]
    fn decide_action_with_public_key_returns_install_key() {
        let status = fresh_status_with_public_key("anything");
        assert_eq!(decide_action(Some(&status)), ClientAction::InstallKey);
    }
    ...
    fn decide_action_with_only_enrollment_code_returns_noop() {
        ...
        assert_eq!(decide_action(Some(&status)), ClientAction::NoOp);
    }
    ...
    fn parse_public_key_b64_returns_none_for_valid_base64_but_invalid_sec1() {
        // Decodes fine; isn't a valid SEC1 point encoding.
    ...
    fn parse_public_key_b64_returns_none_for_empty_string() {
        // Empty string is valid base64 → empty bytes → not a valid key.
        assert!(parse_public_key_b64("").is_none());
    }

    #[test]
    fn build_mint_patch_carries_apiversion_kind_and_name() {
        let patch = build_mint_patch("client-alpha", "code-1", 1_700_000_000);
        assert_eq!(patch["apiVersion"], "sycophant.md/v1");
        assert_eq!(patch["kind"], "Client");
        assert_eq!(patch["metadata"]["name"], "client-alpha");
    }
    ...
    fn build_mint_patch_does_not_set_public_key_or_enrolled_at() {
        // Mint patch must be field-scoped to the two minting fields so
        // SSA doesn't claim ownership over publicKey / enrolledAt and
        // step on redeem_enrollment's writes later.
        let patch = build_mint_patch("c", "code-1", 1_700_000_000);
    ...
    fn build_redeem_patch_carries_apiversion_kind_and_name() {
        let patch = build_redeem_patch("client-alpha", "pk-b64", "2026-05-17T12:00:00Z");
        assert_eq!(patch["apiVersion"], "sycophant.md/v1");
        assert_eq!(patch["kind"], "Client");
        assert_eq!(patch["metadata"]["name"], "client-alpha");
    }
    ...
    fn build_redeem_patch_omits_enrollment_code_fields() {
        // Field-scoped so SSA removes any prior client_watcher-owned
        // enrollmentCode + enrollmentCodeExpiresAt on apply — that's
        // the mechanism enforcing one-time-use.
    ...
    fn default_enrollment_ttl_secs_is_one_hour() {
        // Mutants love changing constants; pin the intent.
        assert_eq!(DEFAULT_ENROLLMENT_TTL_SECS, 3600);
    }

    #[test]
    fn expires_at_adds_ttl_to_now() {
        // Catches `+` → `-` (would yield -3500) and `+` → `*` (would
        // yield 360_000) mutations on the addition.
