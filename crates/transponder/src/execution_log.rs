...
//!  - [`ExecutionLogWriter`] / [`LocalFsExecutionLog`] is the transponder-
...
/// Execution-log store. The transponder is the sole author; the chamber's
...
/// conversation and a re-subscriber filters by id. Transponder-local (the
...
mod tests {
    ...
    // `.frames`, read back by call_id from the transponder-owned store (the
