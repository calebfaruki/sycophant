    ...
    pub(crate) async fn await_client_tool(
        ...
        match handle {
            ...
            None => {
                ...
                // in-memory table, so resolution survives a transponder restart.
...
mod tests {
    ...
    // when it SIGKILLs a mid-flight child; the transponder does not fabricate it.
    ...
    // registry + router over the SAME disk — modelling a transponder restart with
    ...
    // assert.
    #[tokio::test]
    async fn resubscribe_survives_a_transponder_restart_resolving_from_disk_only() {
        use crate::conversation::{ConversationStoreFactory, LocalFsFactory};
        use crate::test_doubles::{FakeAirlock, FakeMainframe};
        use proto_common::tool_result_frame::Frame;
