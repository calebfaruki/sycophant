...
impl ToolClient {
    pub(crate) async fn connect_tcp(addr: &str) -> Result<Self, String> {
        let inner = AirlockControllerClient::connect(addr.to_string())
            .await
            .map_err(|e| format!("failed to connect to tool service at {addr}: {e}"))?;
        Ok(Self { inner })
    }
