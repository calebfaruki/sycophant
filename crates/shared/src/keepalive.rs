...
//! controllers that spawn long-lived capability-job pods and need
...
/// instant its capability job terminates, rather than waiting for the idle
...
mod tests {
    ...
    fn terminal_on_failed_count() {
        // Mutant: flip the `failed > 0` check → a crashed capability job reads as
    ...
    fn not_terminal_when_active() {
        // Running capability job (failed/succeeded zero, condition not True) must
