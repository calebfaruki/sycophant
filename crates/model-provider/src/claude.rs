...
fn content_block_to_api(block: &content_block::Block) -> serde_json::Value {
    match block {
        content_block::Block::Text(t) => serde_json::json!({
            "type": "text",
            "text": t.text,
        }),
        ...
        content_block::Block::Thinking(t) => serde_json::json!({
            "type": "thinking",
            "thinking": t.text,
        }),
        ...
        if m.role == "tool" {
            ...
            while i < messages.len() && messages[i].role == "tool" {
                ...
                if let Some(ref tool_call_id) = tm.tool_call_id {
                    let text = content_text(&tm.content);
...
mod claude_api {
    use super::*;
    use proto_common::{file_block, image_block, text_block, text_content, ToolCall};
    ...
    fn tool_result_converts_to_user_with_tool_result_block() {
        let messages = vec![Message {
            role: "tool".into(),
            content: text_content("file list here"),
            tool_calls: vec![],
    ...
    fn tool_results_with_no_tool_call_ids_emit_placeholder_text() {
        let messages = vec![
            Message {
                role: "tool".into(),
                content: text_content("dropped result"),
                tool_calls: vec![],
                tool_call_id: None,
                is_error: None,
            },
            Message {
                role: "tool".into(),
                content: text_content("also dropped"),
                tool_calls: vec![],
                tool_call_id: None,
                is_error: None,
            },
    ...
    fn tool_result_is_error_propagates_to_block() {
        let messages = vec![Message {
            role: "tool".into(),
            content: text_content("error text"),
            tool_calls: vec![],
    ...
    fn tool_results_dont_swallow_following_assistant_message() {
        let messages = vec![
            ...
            Message {
                role: "assistant".into(),
                content: text_content("ok continuing"),
                tool_calls: vec![],
                tool_call_id: None,
                is_error: None,
            },
    ...
    fn image_block_converts_to_anthropic_format() {
        let messages = vec![Message {
            role: "user".into(),
            content: vec![
                text_block("Describe this".into()),
                image_block("image/png".into(), b"hello".to_vec()),
            ],
            tool_calls: vec![],
            tool_call_id: None,
            is_error: None,
    ...
    fn file_incoming_panics_in_provider() {
        let messages = vec![Message {
            role: "user".into(),
            content: vec![file_block("f.png".into(), "image/png".into(), 1)],
            tool_calls: vec![],
            tool_call_id: None,
            is_error: None,
        }];
