...
use k8s_openapi::api::core::v1::{
    Container, EnvVar, KeyToPath, PodDNSConfig, PodDNSConfigOption, PodSecurityContext, PodSpec,
    PodTemplateSpec, ProjectedVolumeSource, SecretProjection, Volume, VolumeMount,
    VolumeProjection,
};
...
pub fn build_llm_job(
    ...
    let job_name = format!("hangar-llm-{model_name}-{session_id}");
    ...
    let mut env_vars = vec![
        EnvVar {
            name: "HANGAR_CONTROLLER_ADDR".into(),
            value: Some(controller_addr.into()),
            ..Default::default()
        },
        EnvVar {
            name: "HANGAR_MODEL_NAME".into(),
            value: Some(model_name.into()),
            ..Default::default()
        },
        EnvVar {
            name: "HANGAR_JOB_ID".into(),
            value: Some(format!("llm-{model_name}-{session_id}")),
            ..Default::default()
        },
        EnvVar {
            name: "HANGAR_FORMAT".into(),
            value: Some(provider.format.clone()),
            ..Default::default()
        },
        EnvVar {
            name: "HANGAR_MODEL".into(),
            value: Some(model.model.clone()),
            ..Default::default()
        },
        EnvVar {
            name: "HANGAR_BASE_URL".into(),
            value: Some(base_url),
            ..Default::default()
        },
        EnvVar {
            name: "HANGAR_WORKSPACE".into(),
    ...
    // `/run/secrets/hangar/api-key` — never an env var — so the
    ...
    let scrub_entries = serde_json::json!([{
        ...
        "file": "/run/secrets/hangar/api-key",
    }]);
    env_vars.push(EnvVar {
        name: "HANGAR_SCRUB_SECRETS".into(),
    ...
    // value as a file at `/run/secrets/hangar/api-key` (mode 0o440).
    ...
    // HANGAR_API_KEY_PATH (defaulting to that mount path).
    ...
    let secret_volume_name = "hangar-secret".to_string();
    ...
    let secret_mount = VolumeMount {
        ...
        mount_path: "/run/secrets/hangar".into(),
        read_only: Some(true),
        ..Default::default()
    };
    ...
    Job {
        ...
        spec: Some(JobSpec {
            ...
            // would race a second pod for the same assignment against a
            // stale controller view. Fail the Job instead of retrying.
            backoff_limit: Some(0),
            // Coarse platform backstop: bound a wedged/zombie worker even if
            // the reactive watch and idle sweep both miss it. Matches the
            // projected SA-token lifetime (3600s) — past that the worker
            // can't auth to GetTurn/StreamTurnResult anyway.
...
mod tests {
    ...
    const TEST_IMAGE: &str = "ghcr.io/calebfaruki/hangar-llm-job:latest";
    ...
    #[test]
    fn llm_job_sets_ndots_1() {
        // External provider hosts must resolve as-is, not expand to cluster
        // search domains the per-host L7 DNS allowlist denies.
        let dns = sample_llm_job()
            .spec
            .unwrap()
            .template
            .spec
    ...
    #[test]
    fn llm_job_spec_hardening() {
        // A worker holds an in-flight turn, so it must not be silently
        // respawned by K8s (backoff_limit 0), a wedged worker is bounded by
        // the activeDeadline backstop, and finished Jobs are reaped by ttl.
        // Mutant: drop backoff_limit → defaults to 6 → a crashing worker
        // respawns 6× racing the controller's stale view of the slot.
        let job = build_llm_job(
            "claude-sonnet",
            &sample_model_spec(),
            &sample_provider_spec(),
    ...
    fn llm_job_has_correct_name() {
        ...
        assert_eq!(
            ...
            "hangar-llm-claude-sonnet-abc123"
        );
    ...
    fn llm_job_env_vars() {
        ...
        let env = env_map(&job);
        assert_eq!(env["HANGAR_CONTROLLER_ADDR"], "http://controller:9090");
        assert_eq!(env["HANGAR_FORMAT"], "anthropic");
        assert_eq!(env["HANGAR_MODEL"], "claude-sonnet-4-20250514");
        assert_eq!(env["HANGAR_BASE_URL"], "https://api.anthropic.com/v1");
    }
    ...
    fn llm_job_populates_scrub_secrets() {
        ...
        let env = env_map(&job);
        let raw = env
            .get("HANGAR_SCRUB_SECRETS")
            .expect("HANGAR_SCRUB_SECRETS must be set on the LLM job pod");
        let parsed: serde_json::Value =
            serde_json::from_str(raw).expect("HANGAR_SCRUB_SECRETS must parse as JSON");
        ...
        assert_eq!(entries[0]["file"], "/run/secrets/hangar/api-key");
    }
    ...
    fn llm_job_workspace_env_var() {
        ...
        let env = env_map(&job);
        assert_eq!(env["HANGAR_WORKSPACE"], "my-workspace");
    }
    ...
    fn llm_job_secret_mount_path_is_stable() {
        ...
        assert_eq!(mount.mount_path, "/run/secrets/hangar");
    ...
    fn llm_job_no_thinking_env_var() {
        ...
        let env = env_map(&job);
        assert!(
            !env.contains_key("HANGAR_THINKING"),
            "HANGAR_THINKING env var must not be set; thinking moved into pass-through params"
        );
    }
    ...
    fn llm_job_mounts_llm_dispatch_audience_projected_token() {
        ...
        assert_eq!(
            ...
            Some(shared::auth::LLM_HANGAR_AUDIENCE),
            "LLM job token audience must be llm.hangar.sycophant.md \
    ...
    fn llm_job_auth_volume_distinct_from_api_key_volume() {
        ...
        let volume_names: Vec<&str> = pod_spec
            ...
            .map(|v| v.name.as_str())
            .collect();
        assert!(volume_names.contains(&"hangar-secret"));
        ...
        assert!(
            ...
            "hangar-secret projection must not carry a serviceAccountToken"
        );
    }
}
