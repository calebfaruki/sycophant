use crate as proto;
use hangar_providers::types as provider;
...
pub fn stream_event_to_chunk(event: &hangar_providers::StreamEvent) -> proto::TurnResultChunk {
    match event {
        hangar_providers::StreamEvent::ContentDelta { text } => proto::TurnResultChunk {
            chunk: Some(proto::turn_result_chunk::Chunk::ContentDelta(
                proto::ContentDelta { text: text.clone() },
            )),
        },
        hangar_providers::StreamEvent::ToolUseStart { id, name } => proto::TurnResultChunk {
        ...
        hangar_providers::StreamEvent::ToolUseInput { json } => proto::TurnResultChunk {
        ...
        hangar_providers::StreamEvent::ThinkingDelta { .. } => {
            // Thinking deltas are accumulated by the LLM Job, not streamed.
            proto::TurnResultChunk { chunk: None }
        }
        hangar_providers::StreamEvent::Warning { field, reason } => proto::TurnResultChunk {
        ...
        hangar_providers::StreamEvent::Done { stop_reason } => {
...
mod tests {
    ...
    fn stream_event_content_delta_converts() {
        let event = hangar_providers::StreamEvent::ContentDelta {
            text: "Hello".into(),
        };
    ...
    fn stream_event_done_converts() {
        let event = hangar_providers::StreamEvent::Done {
