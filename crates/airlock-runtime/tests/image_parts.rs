...
//!
//!     airlock_runtime::parts::image_part_or_oversize_error(media_type, bytes)
...
fn image_under_cap_becomes_an_image_part_with_media_type_and_bytes() {
    ...
    let block = airlock_runtime::parts::image_part_or_oversize_error("image/png", bytes.clone())
...
fn image_of_exactly_the_cap_size_is_accepted() {
    ...
    let block = airlock_runtime::parts::image_part_or_oversize_error("image/png", bytes)
...
fn image_one_byte_over_the_cap_returns_an_error_not_a_truncated_image() {
    ...
    let result = airlock_runtime::parts::image_part_or_oversize_error("image/png", bytes);
    assert!(
        result.is_err(),
