...
rules:
  ...
  # it in `relay-signing-key` on first start; reads it on restart.
...
When the tsnet bridge is enabled, a second Role grants the SA `get`/`update`/`patch` on the `relay-tsnet-bridge-state` Secret (and `create` for first start) so the sidecar can persist its tailnet identity.
