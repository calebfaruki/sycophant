...
| `mutation-removal.sh <mutation>` | Smoke-test that the chainsaw harness is non-performative: deletes a sycophant policy, runs the affected chainsaw tests (expects RED), restores. Names: `transponder-vap`, `tenant-naming`, `protect-security`, `tenant-perimeter`. |
